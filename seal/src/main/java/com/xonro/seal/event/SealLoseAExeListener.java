...
/**
 * 印章丢失流程结束后，将对应印章的状态改为2(丢失状态)
 *
 * @author hjj
 * @date 2018/1/2
 *
 */
public class SealLoseAExeListener extends ExecuteListener {
    @Override
    public String getDescription() {
        return "印章丢失流程结束后，将对应印章的状态改为2(丢失状态)";
    }
    ...
    public void execute(ProcessExecutionContext cont) throws Exception {
        ...
        if (CONTROLSTATE.equals("end")) {
            // 获取印章编号
            String sealNo = DBSql
                    .getString("select  SEAL_NO from BO_XR_SM_LOSE where BINDID='"
                            + bindId + "'");
            // 根据印章编号修改印章状态为2(丢失状态)
            SealUtil.updateSealFlag(sealNo, 2);
