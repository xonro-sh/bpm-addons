 ...
 * @date  2018-1-24
 */
public class BusinessRBmentProcessAfter extends ExecuteListener {
    @Override
    public String getDescription() {
        return "流程结束后,将业务招待报销金额数据回写到汇总中间表中";
    }
    @Override
    public void execute(ProcessExecutionContext context) throws Exception {
        ...
        if(CONTROLSTATE.equals("end")) {
            ...
            //根据bindId获取业务招待报销子表数据
            List<BO> dataList = SDK.getBOAPI().query("BO_XR_FM_BUSINESS_EXPENSE_S").addQuery("BINDID=", bindId).list();
            if (dataList != null && dataList.size() > 0) {
                BudgetDataDao dao=new BudgetDataDao();
                for (int i = 0; i < dataList.size(); i++) {
                    List<BO> shareList = SDK.getBOAPI().query("BO_XR_FM_BUSINESS_SHARE").addQuery("BINDID=", dataList.get(i).getString("ID")).list();
